...
class CreateInsideRepairTable extends Migration
{
	...
	public function up()
	{
		Schema::create($this->tableName, function (Blueprint $table) {
			...
			$table->integer('exhibit_id',false,true)->comment('藏品id');
			$table->string('exhibit_name',50)->comment('藏品名称');
			...
			$table->string('exhibit_status',50)->comment('文物现状')->nullable();
			$table->string('repair_demand',200)->comment('修复要求')->nullable();
			$table->string('repair_history',200)->comment('修复历史')->nullable();
			$table->string('repair_scheme',200)->comment('修复方案')->nullable();
			$table->string('scheme_editor',50)->comment('方案制定人')->nullable();
			$table->string('scheme_checker',50)->comment('方案审定人')->nullable();
			$table->string('result',50)->comment('校验结果')->nullable();
